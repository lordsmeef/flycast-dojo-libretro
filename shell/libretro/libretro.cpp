...
static bool ggpo_start_attempted = false;
static std::future<bool> ggpo_start_future;
static bool ggpo_last_enable = false;
static bool ggpo_options_logged = false;
...
void retro_init()
{
	...
	{
		std::string base = std::string(save_dir) + "/";
		set_user_data_dir(base);
		set_user_config_dir(base);
	}
...
static bool read_cfg_value(const std::string& path, const std::string& key, std::string& out)
{
	...
	while (std::getline(in, line))
	{
		...
		std::string value = line.substr(key_prefix.size());
...
static bool read_retroarch_core_option_value(const std::string& key, std::string& out)
{
	...
	std::string core_opts = base + slash + "retroarch-core-options.cfg";
	if (read_cfg_value(core_opts, key, out))
		return true;

	std::string retro_cfg = base + slash + "retroarch.cfg";
	std::string core_opts_path;
	if (read_cfg_value(retro_cfg, "core_options_path", core_opts_path) && !core_opts_path.empty())
		return read_cfg_value(core_opts_path, key, out);
...
static void update_ggpo_options(bool first_startup)
{
	...
	bool ggpo_remote_port_set = false;
	const char *ggpo_port_str = nullptr;
	const char *ggpo_remote_port_str = nullptr;
	retro_variable var;

	var.key = CORE_OPTION_NAME "_ggpo_port";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
	{
		ggpo_port_str = var.value;
		int port = atoi(var.value);
		if (port > 0)
		{
			config::GGPOPort = port;
			ggpo_port_set = true;
		}
	}
	...
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
	{
		ggpo_remote_port_str = var.value;
		int port = atoi(var.value);
		if (port > 0)
		{
			config::GGPORemotePort = port;
			ggpo_remote_port_set = true;
		}
	}

	var.key = CORE_OPTION_NAME "_ggpo_delay";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
		config::GGPODelay = atoi(var.value);

	std::string env_ip;
	var.key = CORE_OPTION_NAME "_ggpo_remote_ip";
	if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value && *var.value)
		env_ip = var.value;
...

	{
		...
		if (!ggpo_port_set && port >= 0 && port <= 3)
			port = 19713 + port;
		if (port <= 0)
			port = 19713;
		config::GGPOPort = port;
	...
	{
		int port = config::GGPORemotePort.get();
		if (!ggpo_remote_port_set && port >= 0 && port <= 3)
			port = 19713 + port;
		if (port <= 0)
			port = config::GGPOPort.get();
		config::GGPORemotePort = port;
	}

	if (first_startup || (ggpo_last_enable != (bool)config::GGPOEnable))
	{
		ggpo_last_enable = config::GGPOEnable;
		ggpo_options_logged = false;
		if (log_cb)
			log_cb(RETRO_LOG_INFO, "[GGPO] option=%s\n", ggpo_last_enable ? "enabled" : "disabled");
	}
	if (!ggpo_options_logged && log_cb)
	{
		log_cb(RETRO_LOG_INFO,
			"[GGPO] ports: local=%d (raw=%s) remote=%d (raw=%s)\n",
			config::GGPOPort.get(),
			ggpo_port_str ? ggpo_port_str : "(none)",
...
static void update_variables(bool first_startup)
{
	...
	{
		INFO_LOG(RENDERER, "Threaded rendering disabled for libretro to keep audio/video sync.");
		config::ThreadedRendering = false;
	}

	retro_variable var;
	update_ggpo_options(first_startup);
...
static void maybe_start_ggpo()
{
	...
	if (config::GGPORemotePort.get() <= 0)
		config::GGPORemotePort = config::GGPOPort.get();
	if (log_cb)
		log_cb(RETRO_LOG_INFO, "[GGPO] Starting: role=%s remote_ip=%s port=%d remote_port=%d delay=%d\n",
			config::ActAsServer ? "host" : "client",
			config::NetworkServer.get().c_str(),
			config::GGPOPort.get(),
			config::GGPORemotePort.get(),
			config::GGPODelay.get());
	ggpo_start_future = ggpo::startNetwork();
}

static void poll_ggpo_start_result()
{
	if (!ggpo_start_future.valid())
		return;
	if (ggpo_start_future.wait_for(std::chrono::seconds(0)) != std::future_status::ready)
		return;

	bool ok = false;
	try {
		ok = ggpo_start_future.get();
	} catch (const std::exception& e) {
		ERROR_LOG(COMMON, "GGPO start failed: %s", e.what());
		ggpo_start_attempted = false;
		return;
	} catch (...) {
		ERROR_LOG(COMMON, "GGPO start failed: unknown");
		ggpo_start_attempted = false;
		return;
	}
	if (!ok)
	{
		INFO_LOG(NETWORK, "GGPO start did not activate; ready for retry.");
		ggpo_start_attempted = false;
	}
}
...
void retro_run()
{
	...
	if (first_run)
	{
		emu.start();
		first_run = false;
		if (ggpo_refresh_after_start)
		{
			update_ggpo_options(false);
			ggpo_refresh_after_start = false;
		}
	}
	...
	{
		if (ggpo::isSynchronized())
		{
			if (log_cb)
				log_cb(RETRO_LOG_INFO, "[GGPO] Peer connected; loading netplay state.\n");
			dc_loadstate(-1);
			emu.start();
		...
		{
			if (!ggpo_waiting_notified)
			{
				ggpo_waiting_notified = true;
				gui_display_notification("Waiting for GGPO peer...", 2000);
				if (log_cb)
					log_cb(RETRO_LOG_INFO, "[GGPO] Waiting for peer connection before loading state.\n");
			}
	...
	try {
		...
		ERROR_LOG(COMMON, "C++ exception at emu.render: %s", e.what());
		environ_cb(RETRO_ENVIRONMENT_SHUTDOWN, NULL);
		return;
	} catch (...) {
		ERROR_LOG(COMMON, "C++ exception at emu.render: unknown");
		environ_cb(RETRO_ENVIRONMENT_SHUTDOWN, NULL);
		return;
	}
...
void retro_reset()
{
	...
	emu.unloadGame();
	ggpo_deferred_state_load = config::GGPOEnable;
	ggpo_waiting_notified = false;
...
bool retro_load_game(const struct retro_game_info *game)
{
	...
	ggpo_refresh_after_start = config::GGPOEnable;
	ggpo_deferred_state_load = config::GGPOEnable;
	ggpo_waiting_notified = false;
...
{
   info->library_name = "Flycast Dojo GGPO";
#ifndef GIT_VERSION
#define GIT_VERSION "undefined"
#endif
   info->library_version = GIT_VERSION " (Dojo)";
...
void os_DebugBreak()
{
	ERROR_LOG(COMMON, "DEBUGBREAK (libretro)");
	if (environ_cb)
