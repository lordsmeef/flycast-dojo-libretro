#if !defined(__ANDROID__) && !defined(DOJO_NO_CPR)
#include <cpr/cpr.h>
#endif
...
std::tuple<std::string, std::string> DojoFile::GetLatestDownloadUrl(std::string channel)
{
...
#if !defined(ANDROID) && !defined(DOJO_NO_CURL)
	...
	tag_name = "";
	download_url = "";
	status_text = "Updates disabled (no curl)";
#endif
...
std::string DojoFile::DownloadFile(std::string download_url, std::string dest_folder, size_t download_size, std::string append)
{
	...
	not_found = true;
	download_ended = true;
	status_text = "Downloads disabled (no curl).";
#endif
	of.close();
