...
u16 DojoSession::ApplyOfflineInputs(PlainJoystickState* pjs, u16 buttons, u32 port)
{
	...
	{
		std::string this_frame;
		auto it = net_inputs[port].find(FrameNumber);
		if (it != net_inputs[port].end())
			this_frame = it->second;
		else
			this_frame = current_frame_data;
...
#if !defined(__ANDROID__) && !defined(DOJO_NO_CURL)
