...
static void buildLocalInputs(Inputs &inputs)
{
	inputs = {};
	if (!config::ThreadedRendering)
		UpdateInputState();
...
static const char *ggpo_error_string(GGPOErrorCode code)
{
	switch (code)
	{
	case GGPO_OK:
	...
	case GGPO_ERRORCODE_TOO_MANY_SPECTATORS:
		return "TOO_MANY_SPECTATORS";
	case GGPO_ERRORCODE_INVALID_REQUEST:
		return "INVALID_REQUEST";
	case GGPO_ERRORCODE_UNSUPPORTED:
		return "UNSUPPORTED";
	case GGPO_ERRORCODE_INPUT_SIZE_DIFF:
		return "INPUT_SIZE_DIFF";
	case GGPO_ERRORCODE_VERIFICATION_ERROR:
...
{
	INFO_LOG(NETWORK, "GGPO startSession: local_port=%d local_player=%d", localPort, localPlayerNum);
	...
	INFO_LOG(NETWORK, "GGPO session created");
	...
	player.player_num = (1 - localPlayerNum) + 1;
	INFO_LOG(NETWORK, "GGPO peer: ip=%s port=%u", peerIp.c_str(), peerPort);
	result = ggpo_add_player(ggpoSession, &player, &remotePlayer);
	if (result != GGPO_OK)
	{
...
void getInput(MapleInputState inputState[4])
{
	...
	if (error != GGPO_OK)
	{
		if (error == GGPO_ERRORCODE_NOT_SYNCHRONIZED)
		{
			DEBUG_LOG(NETWORK, "ggpo_synchronize_input not synchronized yet");
			getLocalInput(inputState);
			return;
		}
		WARN_LOG(NETWORK, "ggpo_synchronize_input failed %s (%d)", ggpo_error_string(error), error);
		stopSession();
		throw FlycastException(std::string("GGPO synchronize_input failed: ") + ggpo_error_string(error));
	}
...
bool beginFrame()
{
	...
	Inputs inputs;
	buildLocalInputs(inputs);
	GGPOErrorCode error = ggpo_add_local_input(ggpoSession, localPlayer, &inputs, inputSize);
	if (error == GGPO_OK)
		return true;

	if (error == GGPO_ERRORCODE_NOT_SYNCHRONIZED)
	{
		DEBUG_LOG(NETWORK, "ggpo_add_local_input not synchronized yet");
		error = ggpo_idle(ggpoSession, 0);
		if (error != GGPO_OK)
		{
			WARN_LOG(NETWORK, "ggpo_idle failed %s (%d)", ggpo_error_string(error), error);
			stopSession();
			throw FlycastException(std::string("GGPO idle failed: ") + ggpo_error_string(error));
		}
		return false;
	}
	...
	WARN_LOG(NETWORK, "ggpo_add_local_input failed %s (%d)", ggpo_error_string(error), error);
	stopSession();
	throw FlycastException(std::string("GGPO add_local_input failed: ") + ggpo_error_string(error));
}
...
bool nextFrame()
{
	...
	const char *phase = "advance_frame";
	...
	if (error != GGPO_OK)
	{
		WARN_LOG(NETWORK, "ggpo_%s failed %s (%d)", phase, ggpo_error_string(error), error);
		stopSession();
		if (error == GGPO_ERRORCODE_INPUT_SIZE_DIFF)
			throw FlycastException("GGPO analog settings are different from peer");
		else
			throw FlycastException(std::string("GGPO ") + phase + " failed: " + ggpo_error_string(error));
	}
...
bool isSynchronized()
{
	if (dojo.PlayMatch && dojo.replay_version >= 2)
		return true;
	return ggpoSession != nullptr && synchronized;
}
	...
	return std::async(std::launch::async, []{
		{
			...
			*/
			...
			try {
				...
				INFO_LOG(NETWORK, "GGPO startNetwork: server=%d ip=%s port=%d remote_port=%d delay=%d",
					config::ActAsServer.get(),
		...
		{
			WARN_LOG(NETWORK, "ggpo_idle failed %s (%d)", ggpo_error_string(result), result);
			throw FlycastException(std::string("GGPO error: ") + ggpo_error_string(result));
		}
	}
