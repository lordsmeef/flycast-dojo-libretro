...
static std::string md5_to_hex(const unsigned char digest[16])
{
...
std::string md5file(std::FILE *file)
{
	...
	size_t read_size = 0;
	while ((read_size = std::fread(buffer, 1, sizeof(buffer), file)) > 0)
		MD5_Update(&ctx, buffer, static_cast<unsigned long>(read_size));

	unsigned char digest[16];
	MD5_Final(digest, &ctx);
	return md5_to_hex(digest);
}
...
std::string md5file(const char *filename)
{
	...
	std::string result = md5file(file);
	std::fclose(file);
	return result;
}
